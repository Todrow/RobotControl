...
// Enough for both signed percentages, separators, newline and terminating NUL.
constexpr size_t kFrameCapacity = 32;

size_t formatFrame(char* frame, const WheelPower& power) noexcept {
    const int count = std::snprintf(frame, kFrameCapacity, "R:%d%%|L:%d%%\n",
                                    std::clamp(power.right, -100, 100),
                                    std::clamp(power.left, -100, 100));
