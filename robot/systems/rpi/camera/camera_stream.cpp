...
void runCameraStream(const RobotOptions& options, RobotState& state, const VideoTarget& target) {
    ...
    while (state.running.load()) {
        ...
        bool stream_failed = false;
        {
            ...
            while (!stream_failed && state.running.load() && target.host() == host)
        ...
        if (stream_failed) {
            ...
            while (state.running.load() && target.host() == host && Clock::now() < retry_at)
                std::this_thread::sleep_for(POLL_INTERVAL);
        }
    }
}
