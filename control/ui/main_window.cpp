...
MainWindow::MainWindow(QWidget* parent)
    ...
    // Obstacle warning banner: spans the whole video area, transparent to the
    // mouse, and stays on top so its flashing text is always visible.
    obstacle_overlay_ = new ObstacleOverlay(this);
    obstacle_overlay_->setAttribute(Qt::WA_NativeWindow);
...
void MainWindow::layoutOverlays() {
    ...
    // The obstacle banner covers the whole video area so it can centre itself.
    if (obstacle_overlay_) {
        obstacle_overlay_->setGeometry(area);
        obstacle_overlay_->raise();
        obstacle_overlay_->update();
    }
}
...
void MainWindow::onConnectClicked() {
    if (conn_.running()) {
        ...
        obstacle_overlay_->clearObstacles();
...
void MainWindow::onTelemetry(proto::Telemetry telemetry) {
    ...
    // Flash the warning banner while the robot reports blocked directions.
    obstacle_overlay_->setObstacles(telemetry.obstacles);
    updateQuality();
}
...
void MainWindow::tick() {
    ...
    if (!fresh) {
        hud_->clearTelemetry();
        yaw_view_->clearActualYaw();
        // Stale telemetry: drop the warning rather than freeze it on screen.
        obstacle_overlay_->clearObstacles();
    }
    yaw_view_->setDesiredYaw(slot_.get().camera.yaw);
    updateQuality();
