...
// Obstacle flags, one bit per blocked direction. Sent in telemetry so the
// operator's UI can warn which way is currently blocked. A value of 0 means
// "no obstacles"; bits combine, e.g. FRONT | BACK when boxed in.
enum ObstacleFlags : uint8_t {
    OBSTACLE_NONE  = 0,
    OBSTACLE_FRONT = 1 << 0,
    OBSTACLE_BACK  = 1 << 1,
    OBSTACLE_LEFT  = 1 << 2,
...
struct Telemetry {
    ...
    CameraState camera;
    uint8_t obstacles;  // bitmask of ObstacleFlags; 0 == clear
};
#pragma pack(pop)
...
static_assert(sizeof(Telemetry) == 161);  // 8 + 12*12 + CameraState(8) + obstacles(1)
