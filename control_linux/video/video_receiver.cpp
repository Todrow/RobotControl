...
// xvimagesink embeds most reliably into a foreign X11 window (also under
// XWayland); glimagesink and ximagesink are the fallbacks.
const char* pickSink() {
    for (const char* name : {"xvimagesink", "glimagesink", "ximagesink"}) {
...
bool VideoReceiver::start(quint16 port, WId window) {
    stop();
    window_ = (guintptr)window;  // xcb: WId is the X11 Window id GstVideoOverlay wants
