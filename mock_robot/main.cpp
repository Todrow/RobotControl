...
void printUsage(const char* executable) {
    std::printf(
        ...
        "  --lidar                Enable LDROBOT STL-19P obstacle guard\n"
        "  --lidar-device PATH    LiDAR serial device (default /dev/ttyUSB0)\n"
        "  --lidar-stop-cm N      Stop distance in cm, 2..200 (default 10)\n"
        "  --lidar-sector-deg N   Half-width of front/back sector, 5..90 (default 45)\n"
        "  --help                 Show this help\n"
        ...
        "LiDAR guard blocks FORWARD/BACKWARD when an obstacle is within stop distance.\n"
        "Telemetry is simulated except LiDAR obstacle flags. Camera: rpicam-vid / ksvideosrc.\n",
        executable);
}
...
bool parseOptions(int argc, char* argv[], RobotOptions& options) {
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (name == "--lidar-device") {
            if (value.empty()) {
                std::fprintf(stderr, "--lidar-device must not be empty\n");
                return false;
            }
            options.lidar.device = value;
        } else if (name == "--lidar-stop-cm") {
            if (!integerOption(name, value, 2, 200, number)) return false;
            options.lidar.stop_distance_m = number / 100.0f;
        } else if (name == "--lidar-sector-deg") {
            if (!integerOption(name, value, 5, 90, number)) return false;
            options.lidar.half_width_deg = static_cast<float>(number);
...
int main(int argc, char* argv[]) {
    ...
    // LiDAR obstacle guard: its own thread owns the SDK driver and continuously
    // updates runtime.permissions (read by the command thread) and the obstacle
    // mask (read by the telemetry thread). Inert when --lidar is not given.
    LidarGuard lidar(options.lidar, runtime.permissions);
    if (!lidar.start()) {
        std::fprintf(stderr, "LiDAR enabled but failed to start; continuing without guard\n");
    }
    ...
    std::printf("Drive UART: %s. Camera servos: %s.\n",
                options.drive_uart.enabled ? "ENABLED" : "disabled (use --drive-uart)",
                options.servos.enabled ? "ENABLED" : "disabled (use --servos)");
    std::printf("LiDAR guard: %s. Telemetry: %s.\n",
                options.lidar.enabled ? "ENABLED (front/back obstacle block)"
                                      : "disabled (use --lidar)",
                options.lidar.enabled ? "obstacle flags REAL, rest simulated" : "SIMULATED");
    ...
    try {
        ...
        telemetry = std::thread([&] {
            runSafely("tlm", runtime, [&] { runTelemetrySender(options, runtime, lidar); });
        });
        video = std::thread([&] {
